...
Routes.prototype.publishMessage = function (request, response) {
  ...
  let messages = "messages" in request.body ? request.body.messages : [request.body];

  if (messages.some(message => { return !message.channel && !message.broadcast })) {
  ...
  messages.forEach(message => {
    if (message.broadcast) {
      request.clientManager.broadcastMessage(message);
      sentCount = request.clientManager.getSocketCount();
    }
    else {
      sentCount = request.clientManager.publishMessageToChannel(message);
    }
...
Routes.prototype.healthCheck = function (request, response) {
  ...
  request.clientManager.logger.debug("Health check data.", data);
  response.send(data);
};
...
Routes.prototype.setContentToken = function (request, response) {
  ...
  request.clientManager.logger.debug("Body", request.body);
  request.clientManager.logger.debug("Body type", typeof request.body);

  let tokens = Array.isArray(request.body) ? request.body : [request.body];
  if (tokens.some(token => {
    return !token.channel || !token.token;
  })) {
    response.send({error: "At least one token is missing required parameters."});
  }

  tokens.forEach(token => {
    if (token.channel && token.token) {
      request.clientManager.setContentToken(token.channel, token.token, token);
    }
  })
...
Routes.prototype.publishMessageToContentChannel = function (request, response) {
  ...
  if (messages.some(message => { return !message.channel })) {
    response.send({error: 'Request contains an invalid message.'});
    return;
  }
  ...
  messages.forEach(message => {
    var result = request.clientManager.publishMessageToContentChannel(message.channel, message);
